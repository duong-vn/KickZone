...
export default function FieldsSearchPage() {
  ...
  const pushParams = useCallback(
    (overrides: FilterOverrides) => {
      ...
      const d =
        overrides.district !== undefined ? overrides.district : district;
      const dt = overrides.date !== undefined ? overrides.date : date;
      const ts =
        overrides.timeSlot !== undefined ? overrides.timeSlot : timeSlot;
      const st =
  ...
  const filteredMockData = useMemo(() => {
    ...
    // Lọc theo khu vực (Khớp chính xác quận/huyện)
    if (district && district !== 'Tất cả quận/huyện') {
      result = result.filter((f) => {
        // 1. Nếu mock data có trường district, so sánh chính xác tuyệt đối
        if (f.district) {
          return f.district.toLowerCase() === district.toLowerCase();
        }

        // 2. Nếu tìm trong location, dùng regex để tránh nhầm "Quận 1" với "Quận 10", "Quận 11", "Quận 12"
    ...
    // Logic AND Filter: Sân phải chứa tất cả loại sân được tick
    if (selectedTypes.length > 0) {
      result = result.filter((f) =>
        selectedTypes.every((t) => f.types.includes(t)),
      );
    }
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <aside className="w-full lg:w-1/4">
            <div className="bg-white border border-[#bccbb9]/40 rounded-xl p-5 shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)] space-y-4">
              ...
              {/* Ngày (Custom display dd/mm/yyyy - Click bật popup chọn lịch) */}
              <div>
                <label className="text-xs font-semibold text-[#575e70] block mb-1.5">
                  Ngày
                </label>
          ...
          <main className="w-full lg:w-3/4 space-y-4">
            {/* Header số lượng kết quả & Sắp xếp */}
